...
- [ ] [Joshua] Honest gap, not faked: "always stick to top" (always-on-top) has no QEMU `-display cocoa` flag, and this launcher can't reach into another process's (qemu-system-i386's) Cocoa `NSWindow` to set its window level from a shell script, AppleScript/System Events exposes no window-level property either, only an accessibility-tree view of frontmost/position/size. The only real path is a tiny companion Cocoa/Swift helper that finds the QEMU window by PID after launch and calls `NSWindow.level = .floating` directly, a real small app, not a one-line flag; scoped out of this pass rather than shipped half-working.
