...
/* Ring-3 user-mode tasks (v3 one-shot demo, v64 real lifecycle): copies a
   ...
   scheduled ring-3 task via task_create_user(). Four payloads, picked by
   ...
   task_kill() ends it; "badptr" (test-gap fix, Sep 2026) calls SYS_WRITE
   with buf=NULL, proving sys_write's paging_user_range_ok check actually
   rejects an unmapped user pointer (-EFAULT) instead of only ever being
   exercised on the happy path. Each waits for its task to be reaped and
   reports, the shell comes back every time, no reboot. */
