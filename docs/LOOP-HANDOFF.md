...
Merge recipe: `git merge --no-commit --no-ff origin/main`; conflicts are only VERSION, `landing/version.txt`, `landing/index.html`, and added lines in `docs/roadmap.md` / `docs/ARCHITECTURE.md`. Keep the branch's version (bump if main passed it), `git checkout HEAD -- landing/index.html`, `python3 tools/gen/inject-landing-facts.py`, keep both sides' added doc lines, then `version-bump-check.sh origin/main`, `landing-facts-check.py`, `suite-coverage-check.sh`, `check-refs.sh`, `landing-roadmap.py --check`, `check.sh`, push. Merges are serial because branch protection requires "up to date".
