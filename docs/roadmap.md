...
- [x] 1.0.12: Chat talks to Samantha, the Turing project's model, by default (`llm_model`/`llm_host`/`llm_port` in `kernel.c` point at turing.heyitsmejosh.com's Ollama-shaped `POST /api/chat`); a host that redirects plain HTTP to HTTPS gets a specific status instead of a silent failure; `llmhost=`/`llmport=` boot overrides; `worker.js` forwards only that one host and path; the demo tour's Chat scene sends a real question. Proven against the real host from CI's `network` job. `tools/checks/chat-samantha-check.py`, `tools/checks/chatapp-check.py`.
...
- [x] 1.1.1: CI hygiene. `check.yml`'s gate jobs run under `!cancelled()` instead of `always()`, so a run superseded by a newer push ends as cancelled (no failure email) instead of failed; version names are bare `X.Y.Z` (tags, release titles, ISO names) from 1.0.13 on; the release title no longer repeats the version; the CI manifest's leftover conflict markers (which silently skipped two checks) removed.
