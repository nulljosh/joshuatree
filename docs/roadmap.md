...
- [x] 1.1.2: `check.yml`'s `network` job had been silently failing `wallpaper-check.py`/`wallfx-check.py`/`satellite-wallpaper-check.py` on every real run since commit 1088065 (v0.76.7), hidden by the trailing `|| true`. Root cause, found by bisecting `network` job logs 2026-09-15..2026-09-24 with the GitHub Actions API: 1088065 flipped `wall_theme`'s compiled-in default from `WALL_WARM` to `WALL_SAT` (`kernel/kernel.c`) but never updated `wallpaper-check.py`, which has no explicit `wallpaper map` step and just captures whatever `gui_run`'s automatic first-weather-cycle `wall_fetch` produces -- now Satellite, while the script's own host-side reference still hashes OpenTopoMap. That single mismatch explains both real symptoms: `wallpaper-check.py`'s fnv never matches its topo reference, and `satellite-wallpaper-check.py`'s later, explicit `wallpaper sat` fetch in the same boot reports the exact same fnv as `wallpaper-check.py`'s automatic one (the "same fnv for map and satellite" clue) since both are really the same Satellite fetch for the same runner IP's geolocation. Fixed: new `tilehost=`/`tileport=` command-line override (mirrors `wxhost=`, `kernel/kernel.c`) lets a test point `wall_fetch` at a local fake tile server instead of the real `a.tile.opentopomap.org`/`mt0.google.com`, since this repo's own dev sandbox has no route to either (confirmed directly). New `tools/checks/wallcompose-check.py`, hermetic, drives both themes in one boot against that fake server and proves the Map fnv is byte-correct (PNG, lossless) and never equal to the Satellite fnv -- proven to fail against the pre-fix kernel (no `tilehost=` support at all: real hosts unreachable from this sandbox, fetch errors) and pass against the fix. Separately confirmed (not fixed, out of scope: this is a documented, pre-existing tradeoff, not a regression) that `satellite-wallpaper-check.py`'s own exact-fnv bar for Satellite can never pass as written, real internet or not: `drivers/jpeg.c` is a deliberately simplified, real-but-lossy decoder never claimed to be byte-identical to PIL/libjpeg (see `tools/jpeg-host/main.c`'s own `MAX_DIFF_TOL`/`MEAN_DIFF_TOL`), so `wallcompose-check.py` checks tile coordinates, crop offset and the map/satellite fnv split for that theme instead of exact equality. `check.yml`'s three real-network wallpaper steps keep their `|| true` (a provider hiccup still must not block a push) but now print a loud `::warning::` annotation on a second consecutive failure instead of hiding it silently; the real proof that they pass again on a runner that can actually reach the internet lands on this change's own `network` job run, not from this sandbox.
- [x] 1.1.3: three CodeRabbit reliability follow-ups from the 1.0.11 window-drag PR and the 1.1.0 Chat work. `chat_send`/`chat_pick` (`kernel/chat.h`) now bound their `http_post` reply timeout (new `http_post_timeout`, `drivers/http.c`) instead of inheriting `net.c`'s multi-minute `SLOW_REPLY_TIMEOUT_TICKS` default, so a connected-but-silent LLM host can no longer hold the whole GUI for minutes: ~10s for `/api/pick`, ~45s for `/api/chat`, applied everywhere rather than only for a "looks public" host, since this repo's only headless way to test a silent host (a local fake server) is reachable exclusively through the same private `10.0.2.2` SLIRP gateway a real local Ollama install would also use. `tools/checks/chat-timeout-check.py`. `tools/checks/filerobust-check.py`'s two `subprocess.run` calls (mkdisk, kernel build) get explicit 120s timeouts so a wedged dependency fails the check instead of hanging CI. `tools/checks/clipboard-check.py`'s marker-wait timeout hardened against a real loaded-runner flake (CI job 107600495751): doubled to 30s, plus a length-scaled extra allowance for scenario 3's 116-character line.
...
- [x] 1.0.13: the landing-page demo proves two things a visitor can see instead of just claiming them. The idle tour's Notes scene now really drags its own window by the title bar (press, several small absolute moves, release, then the same drag run backwards so its close light lands back where `runSoloApp`'s own close click expects it) -- a new `drag` script step in `embed.js`'s `runScript`, data like every other step, exercising the exact 1.0.11 title-bar drag live in the demo rather than only in `windowdrag-check.py`. And `tools/checks/demochat-check.mjs` proves the demo's real Chat app (dock slot 7) actually talks to Samantha through the proxied fetch: hermetic (a `page.route` intercept on `/api/proxy`, no real internet), it counts `CHAT_INK` pixels in the reply band straight off the `#screen_canvas` element to prove the reply rendered on screen, not just on serial, and is wired into `.github/workflows/check.yml`'s new `demo` job, which gates merges the same way the suite shards do. `progress.svg` regenerated over the 1.0.7-1.0.12 commits.
