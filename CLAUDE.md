...
- Landing page (`landing/index.html`, deployed to joshuatree.heyitsmejosh.com)
  ...
  driving keystrokes via `osascript ... keystroke`/`key code`. Record text-
  mode and graphics-mode segments separately (the window resizes between
  them), pad the smaller one to the larger one's dimensions before
  concatenating (`ffmpeg -f concat`), encode `libx264`/`yuv420p`/
  `+faststart`. Update `#stage`'s `aspect-ratio` in the CSS to match the
  final video's real dimensions, it won't match the old recording's.
